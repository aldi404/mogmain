                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('event_type') is-invalid @enderror" id="event_type"
                                    ...
                                    <option value="exhibition"
                                        {{ old('event_type', $event->event_type) === 'exhibition' ? 'selected' : '' }}>
                                        Exhibition</option>
                                    <option value="festival"
                                        {{ old('event_type', $event->event_type) === 'festival' ? 'selected' : '' }}>
                                        Festival</option>
                                    <option value="running"
                                        {{ old('event_type', $event->event_type) === 'running' ? 'selected' : '' }}>Running
                                    </option>
                                    <option value="basketball"
                                        {{ old('event_type', $event->event_type) === 'basketball' ? 'selected' : '' }}>
                                        Basketball</option>
                                    <option value="combat_sport"
                                        {{ old('event_type', $event->event_type) === 'combat_sport' ? 'selected' : '' }}>
                                        Combat Sport</option>
                                    <option value="mice"
                                        {{ old('event_type', $event->event_type) === 'mice' ? 'selected' : '' }}>MICE
                                    </option>
                                    <option value="other"
                                        {{ old('event_type', $event->event_type) === 'other' ? 'selected' : '' }}>Other
                                    </option>
                                </select>
                                @error('event_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control @error('registration_fee') is-invalid @enderror"
                                    ...
                                    value="{{ old('registration_fee', $event->registration_fee) }}" min="0"
                                    step="0.01">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                @error('banner_image')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="form-text text-muted">Max size: 2MB. Formats: JPEG, PNG, JPG, GIF. Leave
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" id="status"
                                    ...
                                    <option value="closed"
                                        {{ old('status', $event->status) === 'closed' ? 'selected' : '' }}>Closed</option>
