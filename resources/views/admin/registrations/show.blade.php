        ...
        <div class="col-12">
            <div class="card shadow">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        <span
                            class="badge badge-{{ $registration->status === 'approved' ? 'success' : ($registration->status === 'rejected' ? 'danger' : 'warning') }}">
                ...
                <div class="card-body">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if ($formField->formField->field_type === 'file' && $fieldValue)
                                                            <a href="{{ Storage::url($fieldValue) }}" target="_blank"
                                                    ...
                                                    <tr>
                                                        ...
                                                        @foreach ($section['fields'] as $fieldId)
                                                            @php
                                                                $field = \App\Models\FormField::find($fieldId);
                                                                $fieldValue =
                                                                    $itemData[$field->field_name ?? ''] ?? null;
                                                            @endphp
                                                            <td>
                                                                @if ($field && $fieldValue)
                                                                    @if ($field->field_type === 'file')
                                                                        <a href="{{ Storage::url($fieldValue) }}"
                                                                            target="_blank"
                                                                            class="btn btn-sm btn-outline-primary">
                            ...
                            <div class="card-body">
                                <form method="POST" action="{{ route('admin.registrations.approve', $registration) }}"
                                    ...
                                    <button type="submit" class="btn btn-success"
                                        onclick="return confirm('Setujui data registrasi ini?')">
                    ...
                    @if ($registration->data_approved && $registration->transfer_receipt && !$registration->payment_approved)
                        <div class="card mt-3">
                            ...
                            <div class="card-body">
                                ...
                                <div>
                                    <form method="POST"
                                        action="{{ route('admin.registrations.approve-payment', $registration) }}"
                                        class="d-inline">
                                        @csrf
                                        <button type="submit" class="btn btn-success"
                                    ...
                                    <form method="POST"
                                        action="{{ route('admin.registrations.reject-payment', $registration) }}"
                                        class="d-inline">
                                        @csrf
                                        <button type="submit" class="btn btn-danger"
                    ...
                    @if ($registration->data_approved && $registration->registrationForm->event->registration_fee > 0)
                        <div class="card">
                            ...
                            <div class="card-body">
                                <p><strong>Base Fee:</strong> Rp
                                    {{ number_format($registration->registrationForm->event->registration_fee, 0, ',', '.') }}
                                </p>
                                @if ($registration->unique_amount)
                                    <p><strong>Unique Code:</strong> {{ $registration->unique_amount }}</p>
                                    <p><strong>Total Amount:</strong> <span
                                            class="fw-bold text-primary">{{ $registration->formatted_total_amount }}</span>
                                    </p>
                                @endif
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <div>
                            <a href="{{ route('registrasi.status', $registration->token) }}" class="btn btn-warning"
                                target="_blank">
                                <i class="fas fa-link"></i> Lihat Status Registrasi
                            </a>
                ...
                <form id="statusForm" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="admin_notes" name="admin_notes" rows="3" placeholder="Optional notes...">{{ $registration->admin_notes }}</textarea>
                        </div>
                    </div>
