                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-select" id="event_id" name="event_id">
                            ...
                            @foreach ($registrationForms as $form)
                                <option value="{{ $form->event_id }}"
                                    {{ request('event_id') == $form->event_id ? 'selected' : '' }}>
                                    {{ $form->event->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-select" id="status" name="status">
                            ...
                            <option value="payment_rejected"
                                {{ request('status') === 'payment_rejected' ? 'selected' : '' }}>
                    ...
                    <tbody>
                        @forelse($registrations as $registration)
                            <tr>
                                ...
                                <td>
                                    <div>
                                        ...
                                        @if (isset($registration->participant_data['company']) && $registration->participant_data['company'])
                                            <br>
                                            <small
                                                class="text-muted">{{ $registration->participant_data['company'] }}</small>
                                        @endif
                                        ...
                                        @if ($registration->registrationForm->hasRepeatableSection())
                                            @foreach ($registration->registrationForm->getRepeatableSections() as $section)
                                                @php
                                                    $sectionData =
                                                        $registration->participant_data[$section['name']] ?? [];
                                                @endphp
                                                @if (!empty($sectionData))
                                                    <br>
                                                    <small class="badge bg-info">{{ count($sectionData) }}
                                                        {{ $section['label'] }}</small>
                                                @endif
                                            @endforeach
                                        @endif
                                ...
                                <td>
                                    <span
                                        class="badge bg-{{ $registration->status === 'pending'
                                            ? 'warning'
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        <a href="{{ route('admin.registrations.show', $registration) }}"
                                            class="btn btn-sm btn-outline-info" title="View Registration">
                ...
                <form id="statusForm" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="admin_notes" name="admin_notes" rows="3" placeholder="Optional notes..."></textarea>
                        </div>
                    </div>
